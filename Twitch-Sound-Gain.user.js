...
if (window.TWITCH_SOUND_GAIN === undefined) {
    ...
    (async () => {
        ...
        history.pushState = async function () {
            ...
            abConsole("ROOM"+room);
            if (audioBoosterValueElement != null) audioBoosterValueElement.innerText = await getData("booster_value"+room, 1)+"%";
            if (audioBoosterElement != null) audioBoosterElement.value = await getData("booster_value"+room, 1)*10;
            if (abGainNode != null) abGainNode.gain.value = await getData("booster_value"+room, 1);
        };
